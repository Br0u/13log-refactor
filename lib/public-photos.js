...
async function readWithConnectionRetry(operation) {
  try {
...
export async function getPublicPhotoAlbums() {
  const [categories, photos] = await readWithConnectionRetry(() => Promise.all([
...
export async function getPublicPhotoAlbumBySlug(slug) {
  const category = await readWithConnectionRetry(() => getPhotoCategoryBySlug(slug));
  if (!category || category.status !== "PUBLISHED") {
    return null;
  }

  const photos = sortPhotos(await readWithConnectionRetry(() => listPublishedPhotos({ categorySlug: slug }))).map(mapPhoto);
  if (!photos.length) {
    return null;
  }
