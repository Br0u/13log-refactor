...
export function summarizeAuditLocation(log) {
  const countryCode = normalizeString(log?.country).toUpperCase();
  const region = decodeAuditText(log?.region);
  const city = decodeAuditText(log?.city);
  const secondary = [countryCode, region, city].filter(Boolean).join(" · ") || "Location unavailable";

  return {
    primary: formatAuditLocation(log),
    secondary,
  };
}
...
export function buildAuditDeviceSummary(userAgent = "") {
  const summary = summarizeAuditUserAgent(userAgent);

  return {
    ...summary,
    primary: [summary.browser, summary.os].filter(Boolean).join(" · ") || "未知设备",
    secondary: summary.device || "未知设备类型",
  };
}
...
export async function getAccessAuditPageData(searchParams = {}) {
  ...
  return {
    ...
    rows: rows.map((row) => ({
      ...
      locationSummary: summarizeAuditLocation(row),
      deviceSummary: buildAuditDeviceSummary(row.userAgent),
    })),
  };
}
