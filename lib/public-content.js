...
function microPostNeedsRichHtml(content = "") {
  ...
  return /\r?\n/.test(source) || /<[^>]+>|^\s*[-*+]\s+/m.test(source) || /^\s*\d+\.\s+/m.test(source) || /[`>#]/.test(source) || /\[[^\]]+]\([^)]+\)/.test(source);
}
...
function microPostMatchesTag(post, tag) {
  return entryMatchesTag({
    categories: [],
...
export async function getPublicTimelineEntries(options = {}) {
  ...
  const filteredMicroPosts = activeTag ? microPosts.filter((post) => microPostMatchesTag(post, activeTag)) : microPosts;
...
/**
 * Load the complete public posts page payload with one read of each source.
 *
 * The unrendered projection powers filter-chip counts, while the selected
 * projection is rendered with rich micro HTML only for the active feed.
 */
export async function getPublicPostsPageData(options = {}) {
  const activeTag = typeof options?.tag === "string" && options.tag.trim() ? options.tag.trim() : null;
  const [posts, microPosts] = await Promise.all([
    getPublicPosts(),
    getPublicMicroPosts(),
  ...
  const allMicroPosts = await Promise.all(microPosts.map((post) => mapMicroPost(post, {
    renderRichHtml: false,
  })));
  const allEntries = sortEntriesByDate([
  ...
  const filteredPosts = activeTag ? posts.filter((post) => entryMatchesTag(post, activeTag)) : posts;
  const filteredMicroPosts = activeTag ? microPosts.filter((post) => microPostMatchesTag(post, activeTag)) : microPosts;
  const selectedMicroPosts = await Promise.all(filteredMicroPosts.map((post) => mapMicroPost(post, {
    renderRichHtml: true,
  })));
  const filteredEntries = sortEntriesByDate([
    ...filteredPosts.map(mapTimelinePost),
    ...selectedMicroPosts,
  ]);
