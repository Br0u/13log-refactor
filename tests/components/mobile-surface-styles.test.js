...
describe("mobile surface styles", () => {
  ...
  it("tightens the home hero controls for narrow screens", () => {
    ...
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*720px\)\s*\{[\s\S]*\.profile-avatar\s*\{[^}]*width:\s*min\(16\.8rem,\s*72vw\);/s);
    expect(stylesheet).toMatch(/@media\s*\(max-width:\s*768px\)\s*\{\s*\.profile-avatar\s*\{[^}]*transform:\s*scale\(0\.85\);[^}]*transform-origin:\s*center bottom;/s);
