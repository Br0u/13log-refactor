...
describe("blog rail styles", () => {
  it("creates a wider two-column shell for blog pages and collapses it on mobile", () => {
    ...
    expect(stylesheet).toMatch(/\.blog-layout\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:\s*minmax\(14rem,\s*17rem\)\s+minmax\(0,\s*1fr\);/s);
  ...
  it("styles the shared rail navigation and contextual sections with quiet separators", () => {
    ...
    expect(stylesheet).toMatch(/\.blog-rail__nav\s*\{[^}]*display:\s*grid;[^}]*gap:\s*0\.14rem;[^}]*border:\s*none;/s);
    expect(stylesheet).toMatch(/\.blog-rail__nav-link\s*\{[^}]*border-radius:\s*0\.42rem;[^}]*text-transform:\s*uppercase;[^}]*letter-spacing:\s*0\.045em;/s);
    expect(stylesheet).toMatch(/\.blog-rail__nav-link\.is-active,\s*\.blog-rail__nav-link\[aria-current="page"\]\s*\{[^}]*background:\s*color-mix\(in srgb,\s*var\(--entry\)\s*24%,\s*transparent\);[^}]*box-shadow:\s*none;/s);
  ...
  it("renders the posts intro rail as an asymmetrical image card", () => {
    ...
    expect(stylesheet).toMatch(/\.blog-rail__intro-card\s*\{[^}]*border-radius:\s*0\s+5\.4rem\s+0\s+5\.4rem;/s);
    ...
    expect(stylesheet).toMatch(/\.blog-rail__intro-text\s*\{[^}]*text-align:\s*center;[^}]*letter-spacing:\s*0\.05em;/s);
  });
